...
// Example function call with loader
...
function displayArea(finalresponse) {
    ...
    console.log(dataArray, "from displayArea")

    let cartona = ``
    for (i = 0; i < dataArray.length; i++) {
        let CategoryName = dataArray[i].strArea
        cartona += `
            <div class="col-md-3">
                <div onclick="getMealsOfArea('${CategoryName}')" class="rounded-2 text-center cursor-pointer">
    ...
    document.querySelector("#myAreaData").innerHTML = cartona;
}

async function getMealsOfArea(value) {
    console.log(value)
    console.log("btee5")
    try {
        let response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${value}`)
        let finalresponse = await response.json()
        console.log(finalresponse)
        displayMealsOfArea(finalresponse)
    } catch (error) {

    }
}

function displayMealsOfArea(finalresponse) {
    showLoader()
    let dataArray = finalresponse.meals;
    ...
    for (i = 0; i < dataArray.length; i++) {
        cartona += `
            <div class="col-md-3">
                <div onclick = "displayMealDetailOfArea('${dataArray[i].idMeal}')" class="meal position-relative overflow-hidden rounded-2 cursor-pointer">
...
async function displayMealDetailOfArea(index) {
    showLoader()
